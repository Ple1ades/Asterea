...
std::vector<double> taylor(std::vector<double> taylorFactorsN, std::vector<double> taylorFactorsZ, double deltaX, double deltaY, double velocity)
{
    ...
    for (int i = 0; i < 5; ++i)
    {
        ...
        a[i] = (taylorFactorsN[i] * deltaX - taylorFactorsZ[i] * deltaX * deltaX / (velocity * velocity)) / factorial;
    }
    a[4] -= deltaY;
    return a;
}
double validateSolutions(std::vector<std::complex<double>> solutions, double ang)
{
    double minSol = 1000;
    for (int i = 0; i < 4; ++i)
    {
...
double angleFromPosition(int deltaX, int deltaY, int velocity)
{
    ...
    double deg_15ang = validateSolutions(deg_15sols, 15);
    ...
    double deg_45ang = validateSolutions(deg_45sols, 45);
    ...
    double deg_60ang = validateSolutions(deg_60sols, 60);
    ...
    double deg_75ang = validateSolutions(deg_75sols, 75);
